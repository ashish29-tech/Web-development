...
let allLikeButton = document.querySelectorAll('.like-btn') // .like-btn class de di hai....jitne bhi button ke paas class .like-btn hai..they all will be getting selected inside this variable called like button..


async function likeButton(productId){ //Niche argument bheja hai usey yha catch kar liya.
  // console.log('liked the product');
  //task is to add liked product in the wishlist...for that we'll need that particular id...product id.
...
for(let btn of allLikeButton){ //har ek button pe loop kiya
  btn.addEventListener('click', ()=>{ //btn pe addEventListener lga diya click naam ka...and function run karna hai
    ...
    let productId = btn.getAttribute('product-id'); //So now we have Axios so we can send like ka request is 'product-id' pe.
